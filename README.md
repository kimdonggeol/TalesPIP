...
- **화면 감지** — 게임 화면을 읽어 PIP를 띄울 상황인지 판단
  ...
  - 오픈마켓·아이템샵 등 22가지 창을 기본 제공, 가려야 할 창을 직접 추가도 가능
...
대표적으로 **캐릭터가 접속하기 전에는 PIP가 뜨지 않습니다.**
로그인 화면과 캐릭터 선택창에는 가려줄 게임 화면이 없기 때문입니다.
접속하면 나타나고, 다시 캐릭터 선택창으로 나가면 숨습니다.

오픈마켓, 아이템샵, 옵션창처럼 화면을 크게 차지하는 창 **22가지**가 미리 등록되어
있어서, 그 창을 열면 PIP가 비켜주고 닫으면 돌아옵니다.
이 기준들은 프로그램에 들어 있어 따로 설정할 것이 없습니다.
...
목록의 체크를 끄면 그 조건만 잠시 끌 수 있습니다.
`선택한 조건을 찾을 위치` 를 지정하면 화면 전체 대신 그 구석만 뒤져서 훨씬 빠릅니다.

### 얼마나 빨리 반응하나

| 상황 | 걸리는 시간 |
|---|---|
| 전에 본 자리에서 창이 다시 열릴 때 | 약 0.1초 |
| 그 자리에서 처음 열릴 때 | 1~4초 |

창의 위치는 **해상도별로 기억**해 둡니다. 각 창을 한 번씩 열어보고 나면
그 뒤로는 계속 0.1초입니다. 프로그램을 껐다 켜도 유지됩니다.

창을 드래그해 옮겨도 따라갑니다. 있던 자리에서 사라지면 화면 전체를 다시 훑어
새 위치를 잡습니다.
...
다만 전체화면 독점 모드에서는 동작하지 않습니다. 창모드로 쓰세요.
