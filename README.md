...
- **자동 숨김** — 등록해 둔 그래픽이 게임 화면에 나타나면 PIP를 자동으로 숨김
  - 오픈마켓같은 큰 창을 열 때 PIP가 가리지 않게, 닫으면 다시 나타남
...
오픈마켓처럼 화면을 크게 차지하는 창을 열면 PIP가 거슬리기 마련입니다.
그 창의 고유한 그래픽을 등록해 두면 **창이 떠 있는 동안만 PIP가 자동으로 숨습니다.**

1. 게임에서 그 창을 엽니다
2. 좌측 **자동 숨김 (화면 감지)** 카드에서 **화면에서 추가**를 누릅니다
3. 창 제목처럼 늘 같은 자리에 나오는 부분을 드래그합니다

조건은 두 방향으로 쓸 수 있습니다. 목록에서 고른 뒤 `선택한 조건` 에서 정합니다.

| 방식 | 쓰는 곳 |
|---|---|
| **보이면 숨김** | 오픈마켓처럼 화면을 덮는 창 |
| **보여야 표시** | 퀵슬롯 바처럼 캐릭터가 접속한 뒤에만 나타나는 UI. 로그인 화면에서는 PIP가 뜨지 않게 됩니다 |

`찾는 위치` 를 지정하면 화면 전체 대신 그 구석만 뒤집니다. 퀵슬롯 바를 찾는 경우 좌측 하단만 보면 48ms 에서 6ms 로 줄어듭니다.
